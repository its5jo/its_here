...
public class CategoryService {
    ...
    @PreAuthorize("hasAnyAuthority('MANAGER','MASTER')")
    @Transactional
    public CategoryUpdateResponseDto updateCategory(UUID categoryId, CategoryUpdateRequestDto requestDto) {

        Category category = findCategoryByIdAndNotDeleted(categoryId);

        if (categoryRepository.existsByNameAndIdNotAndDeletedAtIsNull(
                requestDto.name(), categoryId)) {
            throw new ItsHereException(ErrorCode.CATEGORY_NAME_DUPLICATE);
        }

        category.updateCategory(requestDto.name(), requestDto.hasHidden());

        return CategoryUpdateResponseDto.from(category);
    }

    @PreAuthorize("hasAnyAuthority('MANAGER','MASTER')")
    @Transactional
    public void deleteCategory(CustomUserDetails userDetails, UUID categoryId) {
        Category category = findCategoryByIdAndNotDeleted(categoryId);
